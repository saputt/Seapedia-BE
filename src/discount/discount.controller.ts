...
export class DiscountController {
  ...
  @ApiResponse({
    status: 400,
    description: 'Discount expired or not available',
  })
  ...
  async getDiscountForBuyer(@Query('code') code: string) {
    const getDiscountForBuyerResult =
      await this.discountService.getDiscountForBuyer(code);
    return {
  ...
  async getAllDiscountsForAdmin() {
    const getAllDiscountsForAdminResult =
      await this.discountService.getAllDiscountForAdmin();
    return {
  ...
  async getDiscountForAdmin(
    ...
    const getAllDiscountForAdminResult =
      await this.discountService.getDiscountForAdmin(discountId);
    return {
  ...
  async updateDiscount(
    ...
    const updateDiscountResult = await this.discountService.updateDiscount(
      dto,
      discountId,
    );
    return {
