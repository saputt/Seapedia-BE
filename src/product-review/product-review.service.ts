...
export class ProductReviewService {
    ...
    async createReview(productId : string, buyerId : string, dto : CreateProductReviewDto) {
        await this.productService.findProductOrThrow(productId)

        const order = await this.orderService.findOrderOrThrow(dto.orderId)
        if (order.buyerId !== buyerId) throw new BadRequestException("This order does not belong to you")
        if (order.status !== OrderStatus.DELIVERED) throw new BadRequestException("Can only review delivered orders")

        const hasItem = order.orderItems?.some(item => item.productId === productId)
        if (!hasItem) throw new BadRequestException("This product is not in the given order")

        const existing = await this.reviewRepo.findReviewByOrderAndProduct(dto.orderId, productId)
