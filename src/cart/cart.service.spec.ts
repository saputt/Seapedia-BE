...
describe('CartService', () => {
  ...
  describe('addToCart', () => {
    it('should add new product to empty cart', async () => {
      productService.findProductOrThrow.mockResolvedValue({ id: 'p1', storeId: 's1', stock: 10 });
      cartRepo.findUserCartItems.mockResolvedValue([]);
      cartRepo.addToCart.mockResolvedValue({ id: 'c1' });

      await service.addToCart({ quantity: 2 } as any, 'u1', 'p1');
      expect(cartRepo.addToCart).toHaveBeenCalledWith(2, 'p1', 'u1');
    });

    it('should increment quantity if product already in cart', async () => {
      productService.findProductOrThrow.mockResolvedValue({ id: 'p1', storeId: 's1', stock: 10 });
      cartRepo.findUserCartItems.mockResolvedValue([{ id: 'c1', productId: 'p1', quantity: 2, product: { storeId: 's1' } }]);

      await service.addToCart({ quantity: 1 } as any, 'u1', 'p1');
      expect(cartRepo.addQuantityCart).toHaveBeenCalledWith('c1', 1);
    });

    it('should throw when cart has different store products', async () => {
      productService.findProductOrThrow.mockResolvedValue({ id: 'p2', storeId: 's2', stock: 10 });
      cartRepo.findUserCartItems.mockResolvedValue([{ id: 'c1', productId: 'p1', quantity: 1, product: { storeId: 's1' } }]);

      await expect(service.addToCart({ quantity: 1 } as any, 'u1', 'p2')).rejects.toThrow(BadRequestException);
    });

    it('should throw when stock insufficient', async () => {
      productService.findProductOrThrow.mockResolvedValue({ id: 'p1', storeId: 's1', stock: 1 });
      cartRepo.findUserCartItems.mockResolvedValue([]);

      await expect(service.addToCart({ quantity: 5 } as any, 'u1', 'p1')).rejects.toThrow(BadRequestException);
    });
  });
  ...
  describe('updateCartItem', () => {
    it('should update cart item quantity', async () => {
      productService.findProductOrThrow.mockResolvedValue({ id: 'p1', stock: 10 });
      cartRepo.findUserCartItems.mockResolvedValue([{ id: 'c1', productId: 'p1', quantity: 1 }]);
      cartRepo.updateCartItemQuantity.mockResolvedValue({ id: 'c1', quantity: 5 });
    ...
    it('should throw when cart item not found', async () => {
      ...
      await expect(service.updateCartItem('p1', 'u1', 5)).rejects.toThrow(BadRequestException);
    });

    it('should throw when stock insufficient', async () => {
      productService.findProductOrThrow.mockResolvedValue({ id: 'p1', stock: 2 });
      cartRepo.findUserCartItems.mockResolvedValue([{ id: 'c1', productId: 'p1', quantity: 1 }]);
  ...
  describe('deleteCartItem', () => {
    it('should delete cart item', async () => {
      cartRepo.findUserCartItems.mockResolvedValue([{ id: 'c1', productId: 'p1' }]);
    ...
    it('should throw when cart item not found', async () => {
      cartRepo.findUserCartItems.mockResolvedValue([]);
      await expect(service.deleteCartItem('p1', 'u1')).rejects.toThrow(BadRequestException);
    });
  });
});
