...
describe('AuthService', () => {
  ...
  describe('findUserOrThrow', () => {
    ...
    it('should throw UnauthorizedException when not found', async () => {
      ...
      await expect(service.findUserOrThrow('a@b.com')).rejects.toThrow(UnauthorizedException);
    });
  });
  ...
  describe('isEmailAlreadyExist', () => {
    ...
    it('should throw ConflictException when email exists', async () => {
      ...
      await expect(service.isEmailAlreadyExist('a@b.com')).rejects.toThrow(ConflictException);
    });
  });
  ...
  describe('login', () => {
    ...
    it('should throw when password is wrong', async () => {
      repo.findUserByEmail.mockResolvedValue({
        id: 'u1', email: 'a@b.com', password: 'hashed',
        lastActiveRole: 'BUYER', roles: [], username: 'test',
      });
      (hashing.compare as jest.Mock).mockResolvedValue(false);

      await expect(service.login({ email: 'a@b.com', password: 'wrong' })).rejects.toThrow(ForbiddenException);
    });
  });
  ...
  describe('register', () => {
    it('should create user', async () => {
      ...
      const result = await service.register({ email: 'a@b.com', username: 'test', password: 'pass' });
      expect(result.id).toBe('u1');
    });
    ...
    it('should throw ConflictException when email exists', async () => {
      repo.findUserByEmail.mockResolvedValue({ id: 'u1' });
      await expect(
        service.register({ email: 'a@b.com', username: 'test', password: 'pass' }),
  ...
  describe('switchRole', () => {
    it('should switch role and return new token', async () => {
      ...
      const result = await service.switchRole({ role: 'SELLER' } as any, 'a@b.com');
      expect(result.accessToken).toBe('token');
